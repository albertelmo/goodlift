        ...
        if (dateRecords.length > 0 || hasWeight) {
            html += `
                <div class="app-diet-date-section" data-date="${dateStr}">
                    ...
                    ${hasWeight ? `
                    <div class="app-diet-weight-banner" aria-label="몸무게">
                        <div class="app-diet-weight-banner-main">
                            <span class="app-diet-weight-banner-label">몸무게</span>
                            <span class="app-diet-weight-banner-value">${formatWeightDisplay(weightKg)} kg</span>
                        </div>
                        ${!isReadOnly ? `
                        <button type="button" class="app-diet-weight-delete-btn" data-date="${dateStr}" title="몸무게 삭제" aria-label="몸무게 삭제">
...
function setupEventListeners() {
    ...
    container.addEventListener('click', async (e) => {
        ...
        if (weightDeleteBtn && !isReadOnly) {
            ...
            if (!dateStr) return;
            if (!confirm('이 날짜의 몸무게 기록을 삭제할까요?')) {
                return;
            }
            try {
                ...
                alert(error.message || '몸무게 삭제 중 오류가 발생했습니다.');
            }
            return;
        }
