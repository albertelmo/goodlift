...
bodyWeightRecordsDB.initializeDatabase(); // 몸무게 기록 테이블 초기화
...
app.get('/api/body-weight-records/recent', async (req, res) => {
    try {
        ...
        console.error('[API] 몸무게 최근 기록 조회 오류:', error);
        res.status(500).json({ message: '몸무게 기록 조회 중 오류가 발생했습니다.' });
    }
});
...
app.get('/api/body-weight-records/by-date', async (req, res) => {
    try {
        ...
        console.error('[API] 몸무게 날짜별 조회 오류:', error);
        res.status(500).json({ message: '몸무게 기록 조회 중 오류가 발생했습니다.' });
    }
});
...
app.put('/api/body-weight-records', async (req, res) => {
    try {
        ...
        if (!app_user_id || !record_date || weight_kg === undefined || weight_kg === null) {
            return res.status(400).json({ message: '앱 유저 ID, 날짜, 몸무게가 필요합니다.' });
        }
        if (app_user_id.startsWith('trainer-')) {
            return res.status(400).json({ message: '몸무게를 기록할 수 없습니다.' });
        }
        const record = await bodyWeightRecordsDB.upsertRecord(app_user_id, record_date, weight_kg);
        res.json(record);
    } catch (error) {
        console.error('[API] 몸무게 저장 오류:', error);
        const message = error.message || '몸무게 저장 중 오류가 발생했습니다.';
        const status = message.includes('몸무게는') ? 400 : 500;
...
app.delete('/api/body-weight-records/by-date', async (req, res) => {
    try {
        ...
        if (app_user_id.startsWith('trainer-')) {
            return res.status(400).json({ message: '몸무게를 삭제할 수 없습니다.' });
        }
        ...
        console.error('[API] 몸무게 삭제 오류:', error);
        res.status(500).json({ message: '몸무게 삭제 중 오류가 발생했습니다.' });
    }
});
